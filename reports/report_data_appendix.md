...
|---|---|---|---|---|---|---|---|
| M1 FP32 Baseline | 0.0008 | 0.0008 | 5.27 | 100.0 | 100.0 | 0.00012 | 803 |
| M2 PTQ INT8 | 0.0008 | 0.0008 | 4.58 | 98.31 | 100.0 | 0.00012 | 803 |
| M3 Structured-Pruned(35%)+INT8 | 0.0008 | 0.0008 | 3.84 | 98.31 | 100.0 | 0.00012 | 400 |

(Source: `optimisation/results/benchmark_results.csv`, regenerated by
re-running `optimisation/benchmark.py` -- verified stable across 4
repeated runs.)
...
**On the 100.0% vs 98.31% accuracy gap:** the held-out validation set is
only 59 windows, so one flipped prediction is exactly a 1.69pp swing
(1/59) -- not a systematic pattern. M2 and M3 (both INT8) match exactly.
M1's FP32 TFLite conversion classifies one additional borderline window
correctly relative to its own native Keras validation accuracy (98.31%,
Task D1) and relative to the INT8 variants -- plausibly a small numerical-
precision effect from the FP32-to-TFLite conversion path landing a
near-tie sample on the correct side, not a meaningful capability
difference at this dataset size.
...
within one validation sample of the FP32 baseline's accuracy (98.31% vs
100.0% on a 59-window held-out set) and matching 100% Class-2 recall,
but at roughly half the parameter count and the smallest file size of
the three variants. Fewer parameters means fewer FLOPs per inference,
which is the correct lever for a compute-bound workload (Task B2's
Roofline result), and the smallest Flash footprint of the three variants
on the memory-constrained Pi 5 + AI HAT+ edge node. M2 (INT8 only, no
structural change) would be the fallback if a future larger model showed
pruning hurting accuracy -- not the case here, since M2 and M3 score
identically.
